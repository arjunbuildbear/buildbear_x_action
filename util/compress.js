...
async function compressFile(filePath, fileMap, baseDir) {
  try {
    ...
    const decompressed = await gunzip(Buffer.from(fileMap[relativePath].content, 'base64')); // Decompress from base64 string
    const decompressedHash = calculateHash(decompressed);
    
    if (originalHash !== decompressedHash) {
      throw new Error(`Compression validation failed for ${filePath}. Hash mismatch.`);
    }
  } catch (error) {
...
async function compressDirectory(sourceDir, outputDir = os.tmpdir()) {
  try {
    ...
    const metadata = {
      ...
      totalOriginalSize: Object.values(fileMap).reduce((sum, file) => sum + file.originalSize, 0),
      totalCompressedSize: Object.values(fileMap).reduce((sum, file) => sum + file.compressedSize, 0)
    };
    ...
    const compressedArchive = await gzip(serialized, { level: zlib.constants.Z_BEST_COMPRESSION });
    ...
    const outputFile = path.join(outputDir, `${dirName}_compressed_${Date.now()}.gz`);
    ...
    console.log(`Compression ratio: ${(metadata.totalCompressedSize / metadata.totalOriginalSize * 100).toFixed(2)}%`);
...
async function validateCompressedArchive(archivePath, originalFiles, baseDir) {
  try {
    ...
    if (archiveFiles.length !== originalFiles.length) {
      throw new Error(`File count mismatch: Archive has ${archiveFiles.length} files, original had ${originalFiles.length} files`);
    }
    
    // Validate each file
    for (const originalPath of originalFiles) {
      const relativePath = originalPath.replace(new RegExp(`^${baseDir}[/\\\\]?`), '');
      ...
      const decompressedContent = await gunzip(Buffer.from(fileInfo.content, 'base64')); // Decompress from base64 string
